...
public final class InteractiveChatManager {
    ...
    /**
     * Decorates an already-built Adventure component. No serialization round-trip is performed.
     * The cheap token check also makes messages without interactive syntax essentially free.
     */
    public Component decorate(Player sender, Component component) {
        ...
        if (item) {
            ...
            if (count > 0) { Component itemComponent = itemComponent(sender); component = replace(component, "[item]", itemComponent); component = replace(component, "[i]", itemComponent); used += count; }
        }
        if (inventory) {
    ...
    private Component inventoryComponent(Player sender) {
        Component result = plugin.getMessagesManager().get("interactive.inventory-text", "<aqua>[Inventory]</aqua>")
                .hoverEvent(HoverEvent.showText(plugin.getMessagesManager().get("interactive.inventory-hover", "<yellow>Click to view <white>{player}</white>'s inventory.</yellow>", Map.of("player", sender.getName()))));
    ...
    private Component enderComponent(Player sender) {
        Component result = plugin.getMessagesManager().get("interactive.ender-text", "<light_purple>[Ender Chest]</light_purple>")
                .hoverEvent(HoverEvent.showText(plugin.getMessagesManager().get("interactive.ender-hover", "<yellow>Click to view <white>{player}</white>'s Ender Chest.</yellow>", Map.of("player", sender.getName()))));
    ...
    private Component decoratePlayerNames(Component source, Player sender, String plain) {
        Matcher matcher = PLAYER_TOKEN.matcher(plain);
        Set<String> candidates = new LinkedHashSet<>();
        while (matcher.find()) candidates.add(matcher.group(1));
        for (String name : candidates) {
            Player target = Bukkit.getPlayerExact(name);
            if (target == null || target.getUniqueId().equals(sender.getUniqueId())) continue;
            String hover = plugin.getMessagesManager().raw("interactive.player-hover", "<gray>World: <white>{world}</white>\n<gray>Ping: <white>{ping}ms</white>\n<gray>Health: <white>{health}</white>")
                    .replace("{world}", target.getWorld().getName()).replace("{ping}", Integer.toString(target.getPing())).replace("{health}", Integer.toString((int) Math.ceil(target.getHealth())));
    ...
    private Component decorateCommands(Component source) {
        ...
        String serialized = legacy.serialize(source); Matcher matcher = pattern.matcher(serialized); Component result = Component.empty(); int last = 0;
        while (matcher.find()) {
            result = result.append(legacy.deserialize(serialized.substring(last, matcher.start())));
            String command = matcher.group(1); String visible = format.replace("{command}", command);
            result = result.append(miniMessage.deserialize(visible).hoverEvent(HoverEvent.showText(miniMessage.deserialize(hover))).clickEvent(ClickEvent.suggestCommand(command))); last = matcher.end();
        }
    ...
    private Component applyCustomPlaceholders(Component source, Player sender) {
        FileConfiguration cfg = plugin.getPluginConfig(); ConfigurationSection section = cfg.getConfigurationSection("interactive.custom-placeholders");
        if (section == null) return source;
        refreshCustomPatterns(section);
        ...
        for (String id : section.getKeys(false)) {
            ...
            String replace = cfg.getString(path + ".replace", ""); if (!replace.isEmpty()) {
                String parsed = plugin.getServer().getPluginManager().isPluginEnabled("PlaceholderAPI") ? me.clip.placeholderapi.PlaceholderAPI.setPlaceholders(sender, replace) : replace;
                result = result.replaceText(TextReplacementConfig.builder().match(customPatterns.get(id)).replacement(legacy.deserialize(parsed)).build());
            }
        }
        return result;
    }
    private void refreshCustomPatterns(ConfigurationSection section) {
        String signature = section.getKeys(false).stream().map(id -> id + "=" + section.getString(id + ".keyword", "")).sorted().reduce("", (a,b) -> a + "|" + b);
        if (signature.equals(customPatternSignature)) return;
        customPatterns.clear();
        for (String id : section.getKeys(false)) { String regex = section.getString(id + ".keyword", ""); if (!regex.isEmpty()) { try { customPatterns.put(id, Pattern.compile(regex)); } catch (RuntimeException ignored) { } } }
        customPatternSignature = signature;
    }
    private boolean ready(Player player, String key, int cooldownSeconds) {
        if (cooldownSeconds <= 0) return true;
        long now = System.currentTimeMillis(); Map<String, Long> playerCooldowns = cooldowns.computeIfAbsent(player.getUniqueId(), ignored -> new ConcurrentHashMap<>());
        Long previous = playerCooldowns.get(key); if (previous != null && now - previous < cooldownSeconds * 1000L) return false; playerCooldowns.put(key, now); return true;
    }

    public boolean openInventory(Player viewer, UUID targetId) {
        Player target = Bukkit.getPlayer(targetId); if (target == null) return false; PlayerInventory source = target.getInventory(); ReadOnlyHolder holder = new ReadOnlyHolder();
        Inventory view = Bukkit.createInventory(holder, 54, plugin.getMessagesManager().parse("<dark_gray>{player}'s Inventory", Map.of("player", target.getName()))); holder.setInventory(view);
    ...
    public boolean openEnderChest(Player viewer, UUID targetId) {
        Player target = Bukkit.getPlayer(targetId); if (target == null) return false; ReadOnlyHolder holder = new ReadOnlyHolder(); Inventory view = Bukkit.createInventory(holder, 27, plugin.getMessagesManager().parse("<dark_purple>{player}'s Ender Chest", Map.of("player", target.getName())));
        for (int slot = 0; slot < 27; slot++) view.setItem(slot, clone(target.getEnderChest().getItem(slot))); holder.setInventory(view); viewer.openInventory(view); return true;
    }
