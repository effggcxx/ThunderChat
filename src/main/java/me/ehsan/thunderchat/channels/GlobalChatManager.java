...
public final class GlobalChatManager implements PluginMessageListener {
    ...
    public void setHidden(Player player, Channel channel, boolean value) { Set<Channel> channels = hidden.computeIfAbsent(player.getUniqueId(), k -> ConcurrentHashMap.newKeySet()); if (value) channels.add(channel); else channels.remove(channel); if (get(player) == channel && value) active.put(player.getUniqueId(), Channel.LOCAL); if (channels.isEmpty()) hidden.remove(player.getUniqueId(), channels); savePlayerState(player.getUniqueId()); }
    public void toggleHidden(Player player, Channel channel) { setHidden(player, channel, !isHidden(player, channel)); }
    public void hideAll(Player player) { Set<Channel> channels = ConcurrentHashMap.newKeySet(); channels.addAll(Arrays.asList(Channel.values())); hidden.put(player.getUniqueId(), channels); active.put(player.getUniqueId(), Channel.LOCAL); savePlayerState(player.getUniqueId()); }
    ...
    public void send(Player player, String text) {
        ...
        Component interactiveOutput = plugin.getInteractiveChatManager().decorate(player, output);
        for (Player recipient : Bukkit.getOnlinePlayers()) if (shouldReceive(recipient, player.getUniqueId(), channel)) { recipient.sendMessage(interactiveOutput); playMentionSoundIfNeeded(recipient, text, player); }
        if (channel.isNetwork()) forwardChat(player, channel, player.getUniqueId(), interactiveOutput);
    }
    ...
    private String format(String format, Channel channel, String server, String prefix, String player, String message, Player placeholderPlayer) {
        ...
        if (!miniMessageFormat) { resolved = resolved.replace("{prefix}", prefix).replace("{message}", message); return ChatColor.translateAlternateColorCodes('&', resolved); }
        ...
        try { Component component = miniMessage.deserialize(miniFormat, TagResolver.resolver(StandardTags.color(), StandardTags.decorations(), StandardTags.gradient(), resolver)); return legacyOutput.serialize(component); }
        catch (RuntimeException ex) { plugin.getLogger().warning("Invalid MiniMessage channel format: " + format + " (" + ex.getMessage() + ")"); resolved = resolved.replace("{prefix}", prefix).replace("{message}", message); return ChatColor.translateAlternateColorCodes('&', resolved); }
    }
    ...
    private void forwardChat(Player player, Channel channel, UUID senderId, Component resolvedOutput) { try { ByteArrayOutputStream bytes = new ByteArrayOutputStream(); DataOutputStream data = new DataOutputStream(bytes); data.writeInt(PROTOCOL_VERSION); data.writeUTF(PacketKind.CHAT.name()); data.writeUTF(channel.id); data.writeUTF(senderId.toString()); data.writeUTF(gson.serialize(resolvedOutput)); data.flush(); plugin.getNetworkMessenger().forwardAll(player, bytes.toByteArray()); } catch (IOException e) { plugin.getLogger().warning("Could not forward global chat: " + e.getMessage()); } }
