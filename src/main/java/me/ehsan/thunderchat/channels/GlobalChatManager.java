...
public final class GlobalChatManager implements PluginMessageListener {
    public enum Channel { GLOBAL("global","GLOBAL CHAT"), DONATOR("donator","DONATOR CHAT"), STAFF("staff","STAFF CHAT"), ADMIN("admin","ADMIN CHAT"), HIGHRANK("highrank","HIGH RANK CHAT"); final String id,display; Channel(String id,String display){this.id=id;this.display=display;} }
    ...
    public void clearChat(Channel c,Player source){for(Player r:Bukkit.getOnlinePlayers())if(canUse(r,c)&&!r.hasPermission("thunderchat.bypass.clearchat"))ClearChatCommand.sendClear(r);forwardClear(source,c);source.sendMessage(ChatColor.GREEN+"Cleared "+c.display.toLowerCase(Locale.ROOT)+" network chat.");}
