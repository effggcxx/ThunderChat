 ...
 *
 * Channels are registry entries rather than an enum, so adding a channel in
 * config no longer requires recompiling ThunderChat. Built-in constants remain
 * as compatibility aliases for the existing commands and integrations.
 */
public final class GlobalChatManager {
    ...
    public static final class Channel {
        ...
        private static Channel registerConfigured(String id, String display, boolean network, String permission) {
            ...
            REGISTRY.put(channel.id, channel);
            return channel;
        }
    ...
    private void registerConfiguredChannels() {
        ...
        for (String id : section.getKeys(false)) {
            ...
            boolean network = !normalized.equals("local")
                    && plugin.getPluginConfig().getBoolean("channels." + id + ".network", true);
            String permission = plugin.getPluginConfig().getString("channels." + id + ".permission",
                    "thunderchat.channel." + normalized);
    ...
    public boolean isHidden(Player player, Channel channel) {
        Set<Channel> channels = hidden.get(player.getUniqueId());
    ...
    public void hideAll(Player player) {
        ...
        channels.addAll(Arrays.asList(Channel.values()));
        hidden.put(player.getUniqueId(), channels);
        active.put(player.getUniqueId(), Channel.LOCAL);
        savePlayerState(player.getUniqueId());
    }
    ...
    public List<Channel> getAvailableChannels(Player player) {
        ...
        for (Channel channel : Channel.values()) if (canUse(player, channel)) result.add(channel);
    ...
    private void loadState() {
        active.clear();
        hidden.clear();
        String serialized = plugin.getStorage().isEnabled() ? plugin.getStorage().load("channel", "state").join() : null;
        ...
        for (String rawId : players.getKeys(false)) {
            try {
                ...
                for (String hiddenId : data.getStringList("players." + rawId + ".hidden")) {
                    Channel hiddenChannel = Channel.fromId(hiddenId);
    ...
    private boolean shouldReceive(Player recipient, UUID senderId, Channel channel) {
        ...
        return !plugin.getPluginConfig().getBoolean("ignore.public-chat.enabled", false)
            ...
            if (PacketKind.CHAT.name().equals(kind)) {
                ...
                for (Player recipient : Bukkit.getOnlinePlayers()) {
                    if (shouldReceive(recipient, senderId, chatChannel)) {
                        recipient.sendMessage(output);
                        playRemoteMentionSound(recipient, rawMessage, mentionsAllowed);
                    }
                }
                return;
            }
