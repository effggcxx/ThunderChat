...
public final class SpyManager {
    private final ThunderChat plugin; private final Map<UUID,EnumSet<Section>> enabled=new ConcurrentHashMap<>(); private final Set<UUID> initialized=ConcurrentHashMap.newKeySet(); private final File file;
    ...
    public boolean canSpySection(Player player,Section section){return canSpy(player)&&(player.hasPermission("thunderchat.spy.*")||player.hasPermission("thunderchat.spy."+section.name().toLowerCase().replace('_','-')));}
    ...
    public void spyPrivateMessageRemote(UUID sourceId,String sourceName,Player target,String message){if(!plugin.getPluginConfig().getBoolean("spy.enabled",true)||target.hasPermission("thunderchat.bypass.spy"))return;sendLocal(Section.PRIVATE_MESSAGES,format("PM",sourceName+" -> "+target.getName(),sourceName,message),sourceId);}
    ...
    public void spyAnvil(Player source,String text){spyInput(source,Section.ANVILS,"ANVIL",text);} public void spySign(Player source,String text){spyInput(source,Section.SIGNS,"SIGN",text);} public void spyBook(Player source,String text){spyInput(source,Section.BOOKS,"BOOK",text);}
    ...
    private String format(String type,String channel,String player,String message){String path=switch(type){case "PM"->"formats.spy.private-message";case "COMMAND"->"formats.spy.command";case "ANVIL"->"formats.spy.anvil";case "SIGN"->"formats.spy.sign";case "BOOK"->"formats.spy.book";default->"formats.spy.command";};String fallback="<shadow:#000000><bold><dark_gray>[SPY]</dark_gray></bold> <gray>{player}</gray> <dark_gray>:</dark_gray> <gray>{message}</gray></shadow>";Map<String,String> p=new HashMap<>();p.put("type",type);p.put("channel",channel);p.put("player",player);p.put("target",channel.contains(" -> ")?channel.substring(channel.indexOf(" -> ")+4):"");p.put("message",message);return plugin.getMessagesManager().legacy(path,fallback,p);}
    ...
    private void load(){enabled.clear();initialized.clear();String serialized=plugin.getStorage().isEnabled()?plugin.getStorage().load("spy","state").join():null;if(serialized==null&&file.exists()){try{serialized=YamlConfiguration.loadConfiguration(file).saveToString();}catch(Exception ignored){}if(serialized!=null&&plugin.getStorage().isEnabled())plugin.getStorage().put("spy","state",serialized);}if(serialized==null)return;YamlConfiguration data=YamlStorage.parse(serialized);for(String key:data.getKeys(false))try{UUID uuid=UUID.fromString(key);initialized.add(uuid);EnumSet<Section> set=EnumSet.noneOf(Section.class);for(String value:data.getStringList(key))try{set.add(Section.valueOf(value));}catch(IllegalArgumentException ignored){}if(!set.isEmpty())enabled.put(uuid,set);}catch(IllegalArgumentException ignored){}}
    public synchronized void save(){YamlConfiguration data=new YamlConfiguration();for(UUID uuid:initialized){List<String> sections=new ArrayList<>();EnumSet<Section> set=enabled.getOrDefault(uuid,EnumSet.noneOf(Section.class));synchronized(set){for(Section section:set)sections.add(section.name());}data.set(uuid.toString(),sections);}if(plugin.getStorage().isEnabled()){plugin.getStorage().put("spy","state",YamlStorage.serialize(data));return;}try{if(!plugin.getDataFolder().exists())plugin.getDataFolder().mkdirs();data.save(file);}catch(IOException e){plugin.getLogger().warning("Could not save spy settings: "+e.getMessage());}}
}
