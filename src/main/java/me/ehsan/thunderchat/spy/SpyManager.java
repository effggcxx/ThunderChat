...
public final class SpyManager {
    ...
    private String format(String type,String channel,String player,String message){String path=switch(type){case "PM"->"formats.spy.private-message";case "COMMAND"->"formats.spy.command";case "ANVIL"->"formats.spy.anvil";case "SIGN"->"formats.spy.sign";case "BOOK"->"formats.spy.book";default->"formats.spy.command";};String fallback="<shadow:#000000><bold><dark_gray>[SPY]</dark_gray></bold> <gray>{player}</gray> <dark_gray>:</dark_gray> <gray>{message}</gray></shadow>";Map<String,String> placeholders=new HashMap<>();placeholders.put("type",type);placeholders.put("channel",channel);placeholders.put("player",player);placeholders.put("target",channel.contains(" -> ")?channel.substring(channel.indexOf(" -> ")+4):"");placeholders.put("message",message);return plugin.getMessagesManager().legacy(path,fallback,placeholders);}
