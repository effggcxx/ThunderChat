...
public final class MessagesManager {
    ...
    private String replace(String text, Map<String, ?> placeholders) {
        if (placeholders == null) return text;
        Matcher matcher = PLACEHOLDER.matcher(text); StringBuffer result = new StringBuffer();
        while (matcher.find()) { Object value = placeholders.get(matcher.group(1)); matcher.appendReplacement(result, Matcher.quoteReplacement(value == null ? matcher.group(0) : String.valueOf(value))); }
