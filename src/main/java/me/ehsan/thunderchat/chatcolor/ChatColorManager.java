...
public final class ChatColorManager {
    ...
    public void setCustomFormat(Player player, String format) { customFormats.put(player.getUniqueId(), format); colors.remove(player.getUniqueId()); gradients.remove(player.getUniqueId()); styles.remove(player.getUniqueId()); save(); }
