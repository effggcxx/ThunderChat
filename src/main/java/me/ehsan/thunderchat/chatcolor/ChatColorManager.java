...
public final class ChatColorManager {
    ...
    private final Map<UUID, String> customFormats = new HashMap<>();
    private final Set<UUID> awaitingCustomFormat = new HashSet<>();
    ...
    /** Validates a user supplied MiniMessage wrapper. Obfuscated formatting is deliberately forbidden. */
    public boolean isValidCustomFormat(String format) {
        ...
        try {
            miniMessage.deserialize(format + "ThunderChat");
            return true;
    ...
    public void setCustomFormat(Player player, String format) { customFormats.put(player.getUniqueId(), format); colors.remove(player.getUniqueId()); gradients.remove(player.getUniqueId()); save(); }
    ...
    public String colorize(Player player, String message) {
        ...
        String color = getColor(player); String gradient = getGradient(player);
        ...
        StringBuilder closing = new StringBuilder(); List<Style> selected = new ArrayList<>(selectedStyles);
    ...
    private void load() {
        if (!file.exists()) return; YamlConfiguration yaml = YamlConfiguration.loadConfiguration(file); if (yaml.getConfigurationSection("players") == null) return;
        for (String raw : yaml.getConfigurationSection("players").getKeys(false)) try {
            UUID id = UUID.fromString(raw); String color = yaml.getString("players." + raw + ".color"); if (color != null) colors.put(id, color);
            ...
            EnumSet<Style> selected = EnumSet.noneOf(Style.class); for (String style : yaml.getStringList("players." + raw + ".styles")) try { selected.add(Style.valueOf(style.toUpperCase(Locale.ROOT))); } catch (IllegalArgumentException ignored) { } if (!selected.isEmpty()) styles.put(id, selected);
        } catch (IllegalArgumentException ignored) { }
    }
    private synchronized void save() {
        if (!plugin.getDataFolder().exists()) plugin.getDataFolder().mkdirs(); YamlConfiguration yaml = new YamlConfiguration(); Set<UUID> ids = new HashSet<>(); ids.addAll(colors.keySet()); ids.addAll(gradients.keySet()); ids.addAll(customFormats.keySet()); ids.addAll(styles.keySet());
        for (UUID id : ids) { String path = "players." + id; yaml.set(path + ".color", colors.get(id)); yaml.set(path + ".gradient", gradients.get(id)); yaml.set(path + ".custom", customFormats.get(id)); yaml.set(path + ".styles", styles.getOrDefault(id, EnumSet.noneOf(Style.class)).stream().map(Enum::name).map(String::toLowerCase).toList()); }
