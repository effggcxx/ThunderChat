...
public final class MySqlStorage {
    ...
    private void connect() {
        ...
        if (!database.matches("[A-Za-z0-9_$-]+")) {
            enabled = false;
            plugin.getLogger().severe("Invalid storage.mysql.database name: " + database);
            return;
        }

        try {
            // Connect to the server without selecting a schema first. This lets ThunderChat
            // provision the configured database automatically when the MySQL user has CREATE
            // privilege, just like a normal first-run plugin setup.
            String serverUrl = jdbcUrl(host, port, "");
            HikariConfig bootstrap = new HikariConfig();
            bootstrap.setJdbcUrl(serverUrl);
            ...
            try (HikariDataSource bootstrapPool = new HikariDataSource(bootstrap);
                 Connection connection = bootstrapPool.getConnection();
    ...
    private void initialize() throws SQLException {
        try (Connection c = dataSource.getConnection(); Statement s = c.createStatement()) {
            s.executeUpdate("CREATE TABLE IF NOT EXISTS thunderchat_store ("
                    + "namespace VARCHAR(64) NOT NULL, record_key VARCHAR(191) NOT NULL, "
                    + "value LONGTEXT NOT NULL, updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP "
                    + "ON UPDATE CURRENT_TIMESTAMP, PRIMARY KEY(namespace, record_key)) "
                    + "ENGINE=InnoDB DEFAULT CHARSET=utf8mb4");
            s.executeUpdate("CREATE TABLE IF NOT EXISTS thunderchat_network_queue ("
                    + "id BIGINT AUTO_INCREMENT PRIMARY KEY, target_server VARCHAR(128) NOT NULL, "
                    + "payload LONGBLOB NOT NULL, created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP, "
                    + "consumed TINYINT(1) NOT NULL DEFAULT 0, INDEX(target_server, created_at)) "
                    + "ENGINE=InnoDB DEFAULT CHARSET=utf8mb4");
        }
    }
    ...
    private String id(String namespace, String key) {
        ...
        return CompletableFuture.supplyAsync(() -> {
            try (Connection c = dataSource.getConnection();
                 PreparedStatement ps = c.prepareStatement(
                         "SELECT value FROM thunderchat_store WHERE namespace=? AND record_key=?")) {
        ...
        io.execute(() -> {
            try (Connection c = dataSource.getConnection()) {
                ...
                try (PreparedStatement upsert = c.prepareStatement(
                        "INSERT INTO thunderchat_store(namespace,record_key,value) VALUES(?,?,?) "
                                + "ON DUPLICATE KEY UPDATE value=VALUES(value)")) {
                    for (Map.Entry<String, String> entry : batch.entrySet()) {
                        String[] parts = entry.getKey().split("\\u0000", 2);
                        upsert.setString(1, parts[0]);
                        upsert.setString(2, parts[1]);
                        upsert.setString(3, entry.getValue());
                        upsert.addBatch();
                    }
                    upsert.executeBatch();
                }
                try (PreparedStatement del = c.prepareStatement(
                        "DELETE FROM thunderchat_store WHERE namespace=? AND record_key=?")) {
    ...
    public void enqueueNetwork(String targetServer, byte[] payload) {
        ...
        io.execute(() -> {
            try (Connection c = dataSource.getConnection();
                 PreparedStatement ps = c.prepareStatement(
                         "INSERT INTO thunderchat_network_queue(target_server,payload) VALUES(?,?)")) {
    ...
    public void drainNetwork(String serverName, Predicate<byte[]> consumer) {
        ...
        io.execute(() -> {
            ...
            try (Connection c = dataSource.getConnection();
                 PreparedStatement ps = c.prepareStatement(
                         "SELECT id,payload FROM thunderchat_network_queue WHERE target_server=? "
                                 + "AND consumed=0 AND created_at > (CURRENT_TIMESTAMP - INTERVAL ? MINUTE) "
    ...
    private void markConsumed(long id) {
        try (Connection c = dataSource.getConnection();
             PreparedStatement ps = c.prepareStatement(
                     "UPDATE thunderchat_network_queue SET consumed=1 WHERE id=?")) {
            ps.setLong(1, id);
            ps.executeUpdate();
        } catch (SQLException e) {
