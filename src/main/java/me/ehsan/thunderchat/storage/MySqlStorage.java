...
/** Async MySQL persistence/cache layer. Creates the configured database and tables automatically. */
public final class MySqlStorage {
    private final ThunderChat plugin;
    private final ExecutorService io = Executors.newFixedThreadPool(2, r -> {
        Thread t = new Thread(r, "ThunderChat-MySQL"); t.setDaemon(true); return t;
    });
    private final ScheduledExecutorService scheduler = Executors.newSingleThreadScheduledExecutor(r -> {
        Thread t = new Thread(r, "ThunderChat-StorageScheduler"); t.setDaemon(true); return t;
    });
    ...
    private void connect() {
        ...
        if (!database.matches("[A-Za-z0-9_$-]+")) { enabled = false; plugin.getLogger().severe("Invalid storage.mysql.database name: " + database); return; }
        try {
            HikariConfig bootstrap = new HikariConfig();
            bootstrap.setJdbcUrl(jdbcUrl(host, port, "")); bootstrap.setUsername(user); bootstrap.setPassword(password);
            bootstrap.setMaximumPoolSize(1); bootstrap.setMinimumIdle(0); bootstrap.setConnectionTimeout(5000);
            bootstrap.setInitializationFailTimeout(5000); bootstrap.setPoolName("ThunderChat-MySQL-Bootstrap");
            try (HikariDataSource pool = new HikariDataSource(bootstrap); Connection c = pool.getConnection(); Statement s = c.createStatement()) {
                s.executeUpdate("CREATE DATABASE IF NOT EXISTS `" + database + "` CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci");
            }
            HikariConfig config = new HikariConfig();
            config.setJdbcUrl(jdbcUrl(host, port, database)); config.setUsername(user); config.setPassword(password);
            config.setMaximumPoolSize(Math.max(2, plugin.getPluginConfig().getInt("storage.mysql.pool-size", 4)));
            config.setMinimumIdle(1); config.setConnectionTimeout(5000); config.setValidationTimeout(3000);
            config.setIdleTimeout(600000); config.setMaxLifetime(1800000); config.setPoolName("ThunderChat-MySQL");
            dataSource = new HikariDataSource(config); initialize();
            plugin.getLogger().info("MySQL storage enabled. Database '" + database + "' is ready.");
        } catch (Exception ex) {
            enabled = false; plugin.getLogger().severe("Could not initialize MySQL storage: " + ex.getMessage());
            plugin.getLogger().severe("Check the MySQL host, credentials, and that this user can CREATE DATABASE.");
        }
    }
    ...
    public CompletableFuture<String> load(String namespace, String key) {
        ...
        return CompletableFuture.supplyAsync(() -> {
            try (Connection c = dataSource.getConnection(); PreparedStatement ps = c.prepareStatement("SELECT value FROM thunderchat_store WHERE namespace=? AND record_key=?")) {
                ...
                try (ResultSet rs = ps.executeQuery()) { if (!rs.next()) return null; String value = rs.getString(1); cache.put(id(namespace, key), value); return value; }
    ...
    public void put(String namespace, String key, String value) {
        if (!isEnabled()) return; String id = id(namespace, key); cache.put(id, value); dirtyDeletes.remove(id); dirty.put(id, value); scheduleFlush();
    }

    public void delete(String namespace, String key) {
        if (!isEnabled()) return; String id = id(namespace, key); cache.remove(id); dirty.remove(id); dirtyDeletes.add(id); scheduleFlush();
    }
        ...
        io.execute(() -> {
            try (Connection c = dataSource.getConnection()) {
                ...
                try (PreparedStatement upsert = c.prepareStatement("INSERT INTO thunderchat_store(namespace,record_key,value) VALUES(?,?,?) ON DUPLICATE KEY UPDATE value=VALUES(value)")) {
                    for (Map.Entry<String, String> e : batch.entrySet()) { String[] p = e.getKey().split("\\u0000", 2); upsert.setString(1, p[0]); upsert.setString(2, p[1]); upsert.setString(3, e.getValue()); upsert.addBatch(); }
                    upsert.executeBatch();
                }
                try (PreparedStatement del = c.prepareStatement("DELETE FROM thunderchat_store WHERE namespace=? AND record_key=?")) {
                    for (String key : deletes) { String[] p = key.split("\\u0000", 2); del.setString(1, p[0]); del.setString(2, p[1]); del.addBatch(); }
                    del.executeBatch();
                }
                c.commit();
            } catch (SQLException e) { plugin.getLogger().warning("MySQL batch write failed: " + e.getMessage()); dirty.putAll(batch); dirtyDeletes.addAll(deletes); }
        });
    }
    ...
    public void enqueueNetwork(String targetServer, byte[] payload) {
        ...
        io.execute(() -> {
            try (Connection c = dataSource.getConnection(); PreparedStatement ps = c.prepareStatement("INSERT INTO thunderchat_network_queue(target_server,payload) VALUES(?,?)")) {
                ps.setString(1, targetServer); ps.setBytes(2, payload); ps.executeUpdate();
    ...
    public void drainNetwork(String serverName, Predicate<byte[]> consumer) {
        ...
        io.execute(() -> {
            List<Long> ids = new ArrayList<>(); List<byte[]> payloads = new ArrayList<>();
            ...
            try (Connection c = dataSource.getConnection(); PreparedStatement ps = c.prepareStatement("SELECT id,payload FROM thunderchat_network_queue WHERE target_server=? AND consumed=0 AND created_at > (CURRENT_TIMESTAMP - INTERVAL ? MINUTE) ORDER BY id ASC LIMIT 100")) {
                ps.setString(1, serverName); ps.setInt(2, retention);
                try (ResultSet rs = ps.executeQuery()) { while (rs.next()) { ids.add(rs.getLong(1)); payloads.add(rs.getBytes(2)); } }
                for (int i = 0; i < payloads.size(); i++) {
                    long id = ids.get(i); byte[] payload = payloads.get(i);
                    Bukkit.getScheduler().runTask(plugin, () -> {
                        boolean handled;
                        try { handled = consumer.test(payload); } catch (Throwable t) { plugin.getLogger().warning("Queued network packet failed: " + t.getMessage()); handled = false; }
    ...
    private void markConsumed(long id) {
        ...
        } catch (SQLException e) { plugin.getLogger().warning("Could not acknowledge queued network packet: " + e.getMessage()); }
    }

    public void shutdown() {
        flush(); scheduler.shutdownNow(); io.shutdown();
