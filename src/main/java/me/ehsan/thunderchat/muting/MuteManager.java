...
public final class MuteManager implements PluginMessageListener {
    ...
    public MuteManager(ThunderChat plugin) { this.plugin = plugin; this.file = new File(plugin.getDataFolder(), "mutes.yml"); for (String c : channels()) players.put(c, ConcurrentHashMap.newKeySet()); load(); plugin.getServer().getMessenger().registerIncomingPluginChannel(plugin, "BungeeCord", this); }
