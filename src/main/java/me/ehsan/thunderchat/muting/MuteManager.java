...
public final class MuteManager implements PluginMessageListener {
    ...
    /** All known mute channels, in a stable order (for listing / iteration). */
    public List<String> getChannels() {
        return channels();
    }

    /** Channels currently muted for everyone (server-wide, not per-player). */
    public Set<String> getGloballyMutedChannels() {
        return Collections.unmodifiableSet(global);
    }

    /** Per-channel sets of muted players, omitting channels with no player mutes. */
    public Map<String, Set<UUID>> getPlayerMutes() {
        Map<String, Set<UUID>> copy = new LinkedHashMap<>();
        for (Map.Entry<String, Set<UUID>> entry : players.entrySet()) {
