...
public final class ChatChannelCommand implements CommandExecutor {
    ...
    @Override public boolean onCommand(CommandSender sender,Command command,String label,String[] args){
        if(args.length>0&&(args[0].equalsIgnoreCase("mute")||args[0].equalsIgnoreCase("unmute"))){
            ...
            UUID id=target.getUniqueId();plugin.getMuteManager().setPlayerMuted(channel.id,id,muted);sender.sendMessage(ChatColor.GREEN+target.getName()+" has been "+(muted?"muted in ":"unmuted in ")+channel.display+".");return true;
        }
        ...
        else{manager().set(player,channel);player.sendMessage(ChatColor.GREEN+"Chat channel set to "+ChatColor.YELLOW+channel.display+ChatColor.GREEN+".");}
        return true;
    }
}
