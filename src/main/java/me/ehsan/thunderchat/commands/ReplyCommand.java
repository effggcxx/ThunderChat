...
public class ReplyCommand implements CommandExecutor {
    private final ThunderChat plugin; public ReplyCommand(ThunderChat plugin){this.plugin=plugin;}
    @Override public boolean onCommand(CommandSender sender,Command command,String label,String[] args){
        ...
        String message=String.join(" ",args); if(plugin.getFilterManager().shouldBlockPrivateMessage(player,message))return true;
        ...
        String targetName=plugin.getMessageManager().getReplyTargetName(player.getUniqueId());if(targetName==null){plugin.getMessagesManager().send(player,"pm.no-reply","<red>No one to reply to.</red>");return true;}
        if(plugin.getMessageManager().sendReply(player,message)){Player target=plugin.getMessageManager().getReplyTarget(player.getUniqueId());if(target!=null)plugin.getSpyManager().spyPrivateMessage(player,target,message);}else plugin.getMessagesManager().send(player,"pm.target-left","<red>{player} is no longer online.</red>",Map.of("player",targetName));
        return true;
    }
}
