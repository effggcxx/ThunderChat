...
public final class SpyCommand implements CommandExecutor {
    ...
    @Override public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        if (args.length == 0) { player.sendMessage(ChatColor.YELLOW + "Usage: /spy <on|off|status|toggle> [commands|private-messages|anvils|signs|books]"); return true; }
        SpyManager spy = plugin.getSpyManager();
        switch (args[0].toLowerCase()) {
            case "on" -> { spy.enableAll(player); player.sendMessage(ChatColor.GREEN + "Spy enabled for all available sections."); }
            ...
            case "toggle" -> {
                if (args.length < 2) { player.sendMessage(ChatColor.RED + "Usage: /spy toggle <commands|private-messages|anvils|signs|books>"); return true; }
                ...
                if (!spy.canSpySection(player, section)) { player.sendMessage(ChatColor.RED + "You don't have permission to spy on that input type."); return true; }
            ...
            default -> player.sendMessage(ChatColor.RED + "Usage: /spy <on|off|status|toggle> [commands|private-messages|anvils|signs|books]");
        }
        return true;
    }
}
