...
public final class SpyCommand implements CommandExecutor {
    ...
    @Override public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        if (args.length == 0) { player.sendMessage(ChatColor.YELLOW + "Usage: /spy <on|off|status|toggle> [commands|private-messages]"); return true; }
        SpyManager spy = plugin.getSpyManager();
        switch (args[0].toLowerCase()) {
            case "on" -> { spy.enableAll(player); player.sendMessage(ChatColor.GREEN + "Spy enabled for commands and private messages."); }
            ...
            case "toggle" -> {
                if (args.length < 2) { player.sendMessage(ChatColor.RED + "Usage: /spy toggle <commands|private-messages>"); return true; }
            ...
            default -> player.sendMessage(ChatColor.RED + "Usage: /spy <on|off|status|toggle> [commands|private-messages]");
        }
        return true;
    }
}
